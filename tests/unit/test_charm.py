...
def test_generate_dex_auth_config_returns(update_layer, dex_config, harness):
    ...
    # Assert the charm config (generated from dex_config) is the same as the config
    # generated by calling _generate_dex_auth_config()

    # Load the test_configuration so we can retrieve it as it is passed to the dex-auth service
    ...
    # Check if the static username and paswords (str) are the same in the charm config and the service config
    assert harness.model.config['static-username'] == test_configuration_dict['static-username']
    assert harness.model.config['static-password'] == test_configuration_dict['static-password']
